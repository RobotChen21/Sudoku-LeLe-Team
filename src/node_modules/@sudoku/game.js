...
import { hint } from './stores/hint';
import { resetHistory } from './stores/history';
import { parseWikiUrl } from './wiki-parser.js';
...
export function startCustom(sencode) {
	...
	if (typeof sencode === 'string' && 
	    sencode.startsWith('http') && 
	    sencode.includes('bd=')) {
		
		const wikiGrid = parseWikiUrl(sencode);
		if (wikiGrid) {
			grid.updateFromArray(wikiGrid);
		} else {
			// 不是有效的Wiki URL，随机初始化
			startNew(difficulty);
		}
